...
object Haiku {
  ...
  private def randElt[A](xs: List[A]): A = xs.apply(nextInt(xs.size))

  private def randNumber(ra: Range): String = {
    (ra.head + nextInt(ra.end - ra.head)).toString
  }

  def haiku: String = {
    val xs = randNumber(1000 to 9999) :: List(nouns, adjs).map(randElt)
    xs.reverse.mkString("_")
  }
}
